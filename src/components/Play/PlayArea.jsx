...
let markedIndexes = [];
function PlayArea({ player, handleGameOver, scores, updateScores }) {
  ...
  useEffect(() => {
    if (turn !== player) {
      const cpuWinIndex = findBlockChoice(
        markedIndexes[markedIndexes.length - 2],
        turn
      );
      const blockIndex =
        cpuWinIndex < 0
          ? findBlockChoice(markedIndexes[markedIndexes.length - 1], player)
          : cpuWinIndex;
      setTimeout(() => {
        blockIndex < 0 ? markInput(findBestChoice()) : markInput(blockIndex);
        ticTacToeRef.current.style.pointerEvents = "auto";
      }, 1000);
    }
  }, [turn]);
  ...
  useEffect(() => {
    ...
    markedIndexes.length > 0 && turn === player
  ...
  const gameOver = (isWin) => {
    markedIndexes = [];
    turn !== player
      ? (ticTacToeRef.current.style.pointerEvents = "none")
      : null;
    let gameOverProps = {};
    setTimeout(() => {
      isWin
  ...
  const findBlockChoice = useCallback(
    (index, player) => {
      ...
      while (rowIndex < rowEnd && colIndex <= colEnd) {
        inputs[rowIndex] === compareTo
          ? rowCount++
          : inputs[rowIndex] === ""
          ? (rowCpuIndex = rowIndex)
          : null;
        inputs[colIndex] === compareTo
          ? colCount++
          : inputs[colIndex] === ""
          ? (colCpuIndex = colIndex)
          : null;
  ...
  const markInput = (index) => {
    if (!markedIndexes.includes(index)) {
      ...
      turn === player
        ...
        : null;
      setInputs((prev) =>
        prev.map((input, idx) => (idx === index ? turn : input))
      );
    }
  };
  ...
  return (
    <div className="play-area">
      <section className="top-layer">
        ...
        <div
          className="turn"
          style={{ borderColor: turn === "X" ? "#31C4BE" : "#F2B237" }}
        >
          {turn === "X" ? (
            <span>&#215;</span>
          ) : (
            <span style={{ height: "3.8rem" }}>o</span>
          )}
          &nbsp; TURN
        </div>
      ...
      <main className="tic-tac-cards" ref={ticTacToeRef}>
        {inputs.map((card, index) => (
          <div key={index} onClick={() => markInput(index)}>
            {card ? (
              card === "X" ? (
                <Symbol width="50" height="52">
                  X
                </Symbol>
              ) : (
                <Symbol width="53" height="53">
                  O
                </Symbol>
              )
            ) : (
      ...
      <section className="scores">
        <div>
          {player} (YOU) <h3>{scores["user"]}</h3>
        </div>
        <div>
          TIES <h3>{scores["tie"]}</h3>
        </div>
        <div>
          {player === "X" ? "O" : "X"} (CPU) <h3>{scores["cpu"]}</h3>
        </div>
      </section>
    </div>
  );
